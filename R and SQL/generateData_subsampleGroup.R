...
for(i in 1:num_cohorts){
  ...
  createCohortTable(connectionDetails = connectionDetails,
                    cohortDatabaseSchema = cohortDatabaseSchema,
                    cohortTable = paste("SF_Methods_10_", i, sep=""))
  ...
  cmData <- getDbCohortMethodData(connectionDetails = connectionDetails,
                                  ...
                                  exposureTable = paste("SF_Methods_10_", i, sep=""),
...
for(i in 1:num_cohorts){
  if(file.exists(paste(wd, "/cohort_", i, "/cm_df.csv", sep = ""))){
...
for(i in 1:num_cohorts){
  ...
  # Define parameters
  ps <- readRDS(file = paste0(wd, "/cohort_", i, "/Results_1/Ps_l1_s1_p1_t1_c2.rds"))
  cm_df <- read.csv(paste(wd, "/cohort_", i, "/cm_df.csv", sep = ""))
  t_max = 360*60
  t_ind <- cm_df[, "treatment"]
  
  if(!file.exists(paste(wd, paste0("cohort_", i), datafile[7], sep="/"))){
    # Fine matching
...
for(i in 1:num_cohorts){
  ...
  if(!file.exists(paste(wd, paste0("cohort_", i), paste0("balance_", 9, ".rds"), sep="/"))){
    cmData <- loadCohortMethodData(paste(wd, "/cohort_", i, "/cmData", sep = ""))
    studyPop <- createStudyPopulation(cohortMethodData = cmData,
                                      outcomeId = 3,
                                      firstExposureOnly = FALSE,
  ...
  for(j in 1:9){
    ...
    if(nrow(subset(no_matches, cohort == i & cm == (j-2))) != 0){
      print(paste("No matches for cohort", i, "data file", datafile[j-2], "- skipping"))
      next
    } else if(nrow(subset(unbal_matches, cohort == i & cm == (j-2))) != 0){
    ...
    if(!file.exists(paste(wd, paste0("cohort_", i), paste0("balance_", j, ".rds"), sep="/"))){
      if(j == 1 | j == 2){
        ...
        cm_df <- read.csv(paste(wd, paste0("cohort_", i), "cm_df.csv", sep="/"))
